#!/usr/bin/env bash
# ct/oauth2-proxy.sh — Oauth2 Proxy
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/oauth2-proxy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for oauth2-proxy"
    exit 1
}
...
echo ""
echo -e "${GR}Oauth2 Proxy deployed on ${var_instance} (${IP})${NC}"
echo ""
