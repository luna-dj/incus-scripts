...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/sync-in-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for sync-in"
    exit 1
}
...
echo ""
echo -e "${GR}Sync In deployed on ${var_instance} (${IP})${NC}"
echo ""
