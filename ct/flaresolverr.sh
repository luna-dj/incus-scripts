#!/usr/bin/env bash
# ct/flaresolverr.sh — Flaresolverr
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/flaresolverr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for flaresolverr"
    exit 1
}
...
echo ""
echo -e "${GR}Flaresolverr deployed on ${var_instance} (${IP})${NC}"
echo ""
