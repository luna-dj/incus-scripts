#!/usr/bin/env bash
# ct/droppedneedle.sh — Droppedneedle
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/droppedneedle-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for droppedneedle"
    exit 1
}
...
echo ""
echo -e "${GR}Droppedneedle deployed on ${var_instance} (${IP})${NC}"
echo ""
