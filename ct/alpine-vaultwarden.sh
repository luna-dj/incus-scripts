#!/usr/bin/env bash
# ct/alpine-vaultwarden.sh — Alpine Vaultwarden
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-vaultwarden-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-vaultwarden"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Vaultwarden deployed on ${var_instance} (${IP})${NC}"
echo ""
