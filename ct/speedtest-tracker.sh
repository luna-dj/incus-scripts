#!/usr/bin/env bash
# ct/speedtest-tracker.sh — Speedtest Tracker
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/speedtest-tracker-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for speedtest-tracker"
    exit 1
}
...
echo ""
echo -e "${GR}Speedtest Tracker deployed on ${var_instance} (${IP})${NC}"
echo ""
