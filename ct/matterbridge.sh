#!/usr/bin/env bash
# ct/matterbridge.sh — Matterbridge
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/matterbridge-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for matterbridge"
    exit 1
}
...
echo ""
echo -e "${GR}Matterbridge deployed on ${var_instance} (${IP})${NC}"
echo ""
