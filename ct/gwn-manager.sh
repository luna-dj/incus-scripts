...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/gwn-manager-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for gwn-manager"
    exit 1
}
...
echo ""
echo -e "${GR}Gwn Manager deployed on ${var_instance} (${IP})${NC}"
echo ""
