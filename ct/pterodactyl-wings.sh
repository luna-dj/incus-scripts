#!/usr/bin/env bash
# ct/pterodactyl-wings.sh — Pterodactyl Wings
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/pterodactyl-wings-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pterodactyl-wings"
    exit 1
}
...
echo ""
echo -e "${GR}Pterodactyl Wings deployed on ${var_instance} (${IP})${NC}"
echo ""
