...
#!/usr/bin/env bash
# ct/dokploy.sh — Dokploy (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/dokploy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for dokploy"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "dokploy"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Dokploy deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Dokploy install did not start a Docker container on ${var_instance} (${IP}).${NC}"
