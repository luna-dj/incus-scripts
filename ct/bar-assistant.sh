#!/usr/bin/env bash
# ct/bar-assistant.sh — Bar Assistant
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/bar-assistant-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for bar-assistant"
    exit 1
}
...
echo ""
echo -e "${GR}Bar Assistant deployed on ${var_instance} (${IP})${NC}"
echo ""
