#!/usr/bin/env bash
# ct/domain-monitor.sh — Domain Monitor
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/domain-monitor-install.sh" 2>/dev/null) || {
...
echo ""
echo -e "${GR}Domain Monitor deployed on ${var_instance} (${IP})${NC}"
echo ""
