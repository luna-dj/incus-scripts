#!/usr/bin/env bash
# ct/alpine-traefik.sh — Alpine Traefik
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-traefik-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-traefik"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Traefik deployed on ${var_instance} (${IP})${NC}"
echo ""
