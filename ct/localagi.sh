...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/localagi-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for localagi"
    exit 1
}
...
echo ""
echo -e "${GR}Localagi deployed on ${var_instance} (${IP})${NC}"
echo ""
