...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/opensuse-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for opensuse"
    exit 1
}
...
echo ""
echo -e "${GR}Opensuse deployed on ${var_instance} (${IP})${NC}"
echo ""
