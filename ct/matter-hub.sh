...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/matter-hub-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for matter-hub"
    exit 1
}
...
echo ""
echo -e "${GR}Matter Hub deployed on ${var_instance} (${IP})${NC}"
echo ""
