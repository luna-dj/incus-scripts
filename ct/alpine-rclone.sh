#!/usr/bin/env bash
# ct/alpine-rclone.sh — Alpine Rclone
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-rclone-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-rclone"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Rclone deployed on ${var_instance} (${IP})${NC}"
echo ""
