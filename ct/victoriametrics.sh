#!/usr/bin/env bash
# ct/victoriametrics.sh — Victoriametrics
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/victoriametrics-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for victoriametrics"
    exit 1
}
...
echo ""
echo -e "${GR}Victoriametrics deployed on ${var_instance} (${IP})${NC}"
echo ""
