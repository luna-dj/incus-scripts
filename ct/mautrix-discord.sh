#!/usr/bin/env bash
# ct/mautrix-discord.sh — mautrix-discord
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mautrix-discord.sh)
...
APP="mautrix-discord"
var_tags="${var_tags:-matrix;bridge;discord}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-512}"
var_disk="${var_disk:-4}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mautrix-discord-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mautrix-discord"
    exit 1
}
...
echo ""
echo -e "${GR}mautrix-discord deployed on ${var_instance} (${IP})${NC}"
echo ""
