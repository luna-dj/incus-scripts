#!/usr/bin/env bash
# ct/apache-guacamole.sh — Apache Guacamole
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/apache-guacamole-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for apache-guacamole"
    exit 1
}
...
echo ""
echo -e "${GR}Apache Guacamole deployed on ${var_instance} (${IP})${NC}"
echo ""
