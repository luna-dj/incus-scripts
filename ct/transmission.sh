#!/usr/bin/env bash
# ct/transmission.sh — Transmission
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/transmission-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for transmission"
    exit 1
}
...
echo ""
echo -e "${GR}Transmission deployed on ${var_instance} (${IP})${NC}"
echo ""
