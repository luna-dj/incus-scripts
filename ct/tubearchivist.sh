#!/usr/bin/env bash
# ct/tubearchivist.sh — Tubearchivist
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/tubearchivist-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for tubearchivist"
    exit 1
}
...
echo ""
echo -e "${GR}Tubearchivist deployed on ${var_instance} (${IP})${NC}"
echo ""
