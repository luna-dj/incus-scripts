...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/devuan-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for devuan"
    exit 1
}
...
echo ""
echo -e "${GR}Devuan deployed on ${var_instance} (${IP})${NC}"
echo ""
