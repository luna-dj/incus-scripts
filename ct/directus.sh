...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/directus-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for directus"
    exit 1
}
...
echo ""
echo -e "${GR}Directus deployed on ${var_instance} (${IP})${NC}"
echo ""
