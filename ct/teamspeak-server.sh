#!/usr/bin/env bash
# ct/teamspeak-server.sh — Teamspeak Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/teamspeak-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for teamspeak-server"
    exit 1
}
...
echo ""
echo -e "${GR}Teamspeak Server deployed on ${var_instance} (${IP})${NC}"
echo ""
