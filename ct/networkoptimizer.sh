#!/usr/bin/env bash
# ct/networkoptimizer.sh — Networkoptimizer
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/networkoptimizer-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for networkoptimizer"
    exit 1
}
...
echo ""
echo -e "${GR}Networkoptimizer deployed on ${var_instance} (${IP})${NC}"
echo ""
