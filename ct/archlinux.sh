...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/archlinux-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for archlinux"
    exit 1
}
...
echo ""
echo -e "${GR}Archlinux deployed on ${var_instance} (${IP})${NC}"
echo ""
