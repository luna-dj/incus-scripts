#!/usr/bin/env bash
# ct/alpine-syncthing.sh — Alpine Syncthing
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-syncthing-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-syncthing"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Syncthing deployed on ${var_instance} (${IP})${NC}"
echo ""
