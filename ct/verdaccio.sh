...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/verdaccio-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for verdaccio"
    exit 1
}
...
echo ""
echo -e "${GR}Verdaccio deployed on ${var_instance} (${IP})${NC}"
echo ""
