...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/airtrail-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for airtrail"
    exit 1
}
...
echo ""
echo -e "${GR}Airtrail deployed on ${var_instance} (${IP})${NC}"
echo ""
