#!/usr/bin/env bash
# ct/apache-couchdb.sh — Apache Couchdb
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/apache-couchdb-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for apache-couchdb"
    exit 1
}
...
echo ""
echo -e "${GR}Apache Couchdb deployed on ${var_instance} (${IP})${NC}"
echo ""
