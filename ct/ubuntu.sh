...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/ubuntu-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for ubuntu"
    exit 1
}
...
echo ""
echo -e "${GR}Ubuntu deployed on ${var_instance} (${IP})${NC}"
echo ""
