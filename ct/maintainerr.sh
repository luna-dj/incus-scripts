...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/maintainerr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for maintainerr"
    exit 1
}
...
echo ""
echo -e "${GR}Maintainerr deployed on ${var_instance} (${IP})${NC}"
echo ""
