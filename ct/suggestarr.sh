...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/suggestarr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for suggestarr"
    exit 1
}
...
echo ""
echo -e "${GR}Suggestarr deployed on ${var_instance} (${IP})${NC}"
echo ""
