...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/invidious-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for invidious"
    exit 1
}
...
echo ""
echo -e "${GR}Invidious deployed on ${var_instance} (${IP})${NC}"
echo ""
