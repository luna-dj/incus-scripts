#!/usr/bin/env bash
# ct/actualbudget.sh — Actualbudget
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/actualbudget-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for actualbudget"
    exit 1
}
...
echo ""
echo -e "${GR}Actualbudget deployed on ${var_instance} (${IP})${NC}"
echo ""
