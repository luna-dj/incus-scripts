...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/lingarr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for lingarr"
    exit 1
}
...
echo ""
echo -e "${GR}Lingarr deployed on ${var_instance} (${IP})${NC}"
echo ""
