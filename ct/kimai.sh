...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/kimai-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for kimai"
    exit 1
}
...
echo ""
echo -e "${GR}Kimai deployed on ${var_instance} (${IP})${NC}"
echo ""
