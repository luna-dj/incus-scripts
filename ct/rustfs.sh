...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/rustfs-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for rustfs"
    exit 1
}
...
echo ""
echo -e "${GR}Rustfs deployed on ${var_instance} (${IP})${NC}"
echo ""
