...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/etherpad-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for etherpad"
    exit 1
}
...
echo ""
echo -e "${GR}Etherpad deployed on ${var_instance} (${IP})${NC}"
echo ""
