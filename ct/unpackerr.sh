...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/unpackerr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for unpackerr"
    exit 1
}
...
echo ""
echo -e "${GR}Unpackerr deployed on ${var_instance} (${IP})${NC}"
echo ""
