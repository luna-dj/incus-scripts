...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/fmd-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for fmd-server"
    exit 1
}
...
echo ""
echo -e "${GR}Fmd Server deployed on ${var_instance} (${IP})${NC}"
echo ""
