#!/usr/bin/env bash
# ct/pelican-panel.sh — Pelican Panel
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/pelican-panel-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pelican-panel"
    exit 1
}
...
echo ""
echo -e "${GR}Pelican Panel deployed on ${var_instance} (${IP})${NC}"
echo ""
