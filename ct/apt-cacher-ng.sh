#!/usr/bin/env bash
# ct/apt-cacher-ng.sh — Apt Cacher Ng
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/apt-cacher-ng-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for apt-cacher-ng"
    exit 1
}
...
echo ""
echo -e "${GR}Apt Cacher Ng deployed on ${var_instance} (${IP})${NC}"
echo ""
