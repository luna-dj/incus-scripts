#!/usr/bin/env bash
# ct/isponsorblocktv.sh — Isponsorblocktv
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/isponsorblocktv-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for isponsorblocktv"
    exit 1
}
...
echo ""
echo -e "${GR}Isponsorblocktv deployed on ${var_instance} (${IP})${NC}"
echo ""
