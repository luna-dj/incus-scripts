#!/usr/bin/env bash
# ct/elementsynapse.sh — Elementsynapse
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/elementsynapse-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for elementsynapse"
    exit 1
}
...
echo ""
echo -e "${GR}Elementsynapse deployed on ${var_instance} (${IP})${NC}"
echo ""
