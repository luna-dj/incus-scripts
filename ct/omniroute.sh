...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/omniroute-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for omniroute"
    exit 1
}
...
echo ""
echo -e "${GR}Omniroute deployed on ${var_instance} (${IP})${NC}"
echo ""
