...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/pinchflat-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pinchflat"
    exit 1
}
...
echo ""
echo -e "${GR}Pinchflat deployed on ${var_instance} (${IP})${NC}"
echo ""
