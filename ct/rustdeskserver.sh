#!/usr/bin/env bash
# ct/rustdeskserver.sh — Rustdeskserver
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/rustdeskserver-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for rustdeskserver"
    exit 1
}
...
echo ""
echo -e "${GR}Rustdeskserver deployed on ${var_instance} (${IP})${NC}"
echo ""
