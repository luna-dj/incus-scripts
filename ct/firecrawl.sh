...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/firecrawl-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for firecrawl"
    exit 1
}
...
echo ""
echo -e "${GR}Firecrawl deployed on ${var_instance} (${IP})${NC}"
echo ""
