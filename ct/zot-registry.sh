#!/usr/bin/env bash
# ct/zot-registry.sh — Zot Registry
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/zot-registry-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for zot-registry"
    exit 1
}
...
echo ""
echo -e "${GR}Zot Registry deployed on ${var_instance} (${IP})${NC}"
echo ""
