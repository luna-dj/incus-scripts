...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/discourse-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for discourse"
    exit 1
}
...
echo ""
echo -e "${GR}Discourse deployed on ${var_instance} (${IP})${NC}"
echo ""
