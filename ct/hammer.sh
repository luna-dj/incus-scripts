...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/hammer-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for hammer"
    exit 1
}
...
echo ""
echo -e "${GR}Hammer deployed on ${var_instance} (${IP})${NC}"
echo ""
