...
#!/usr/bin/env bash
# ct/arcane.sh — Arcane (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/arcane-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for arcane"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "arcane"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Arcane deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Arcane install did not start a Docker container on ${var_instance} (${IP}).${NC}"
