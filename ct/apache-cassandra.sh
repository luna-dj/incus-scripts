#!/usr/bin/env bash
# ct/apache-cassandra.sh — Apache Cassandra
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/apache-cassandra-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for apache-cassandra"
    exit 1
}
...
echo ""
echo -e "${GR}Apache Cassandra deployed on ${var_instance} (${IP})${NC}"
echo ""
