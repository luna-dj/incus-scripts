#!/usr/bin/env bash
# ct/heisenbridge.sh — Heisenbridge (IRC)
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/heisenbridge.sh)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/heisenbridge-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for heisenbridge"
    exit 1
}
...
echo ""
echo -e "${GR}Heisenbridge (IRC) deployed on ${var_instance} (${IP})${NC}"
echo ""
