...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/teslamate-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for teslamate"
    exit 1
}
...
echo ""
echo -e "${GR}Teslamate deployed on ${var_instance} (${IP})${NC}"
echo ""
