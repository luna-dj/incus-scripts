#!/usr/bin/env bash
# ct/github-runner.sh — Github Runner
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/github-runner-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for github-runner"
    exit 1
}
...
echo ""
echo -e "${GR}Github Runner deployed on ${var_instance} (${IP})${NC}"
echo ""
