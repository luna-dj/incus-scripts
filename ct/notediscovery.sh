#!/usr/bin/env bash
# ct/notediscovery.sh — Notediscovery
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/notediscovery-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for notediscovery"
    exit 1
}
...
echo ""
echo -e "${GR}Notediscovery deployed on ${var_instance} (${IP})${NC}"
echo ""
