#!/usr/bin/env bash
# ct/onetimesecret.sh — Onetimesecret
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/onetimesecret-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for onetimesecret"
    exit 1
}
...
echo ""
echo -e "${GR}Onetimesecret deployed on ${var_instance} (${IP})${NC}"
echo ""
