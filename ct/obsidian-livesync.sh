#!/usr/bin/env bash
# ct/obsidian-livesync.sh — Obsidian Livesync
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/obsidian-livesync-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for obsidian-livesync"
    exit 1
}
...
echo ""
echo -e "${GR}Obsidian Livesync deployed on ${var_instance} (${IP})${NC}"
echo ""
