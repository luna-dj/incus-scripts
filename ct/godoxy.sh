...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/godoxy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for godoxy"
    exit 1
}
...
echo ""
echo -e "${GR}Godoxy deployed on ${var_instance} (${IP})${NC}"
echo ""
