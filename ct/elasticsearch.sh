#!/usr/bin/env bash
# ct/elasticsearch.sh — Elasticsearch
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/elasticsearch-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for elasticsearch"
    exit 1
}
...
echo ""
echo -e "${GR}Elasticsearch deployed on ${var_instance} (${IP})${NC}"
echo ""
