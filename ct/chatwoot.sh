...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/chatwoot-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for chatwoot"
    exit 1
}
...
echo ""
echo -e "${GR}Chatwoot deployed on ${var_instance} (${IP})${NC}"
echo ""
