...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/jdownloader-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for jdownloader"
    exit 1
}
...
echo ""
echo -e "${GR}Jdownloader deployed on ${var_instance} (${IP})${NC}"
echo ""
