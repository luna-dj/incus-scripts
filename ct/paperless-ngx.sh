#!/usr/bin/env bash
# ct/paperless-ngx.sh — Paperless Ngx
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/paperless-ngx-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for paperless-ngx"
    exit 1
}
...
echo ""
echo -e "${GR}Paperless Ngx deployed on ${var_instance} (${IP})${NC}"
echo ""
