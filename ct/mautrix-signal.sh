#!/usr/bin/env bash
# ct/mautrix-signal.sh — mautrix-signal
# Matrix bridge to Signal.
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mautrix-signal.sh)
...
APP="mautrix-signal"
var_tags="${var_tags:-matrix;bridge;signal}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-512}"
var_disk="${var_disk:-4}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mautrix-signal-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mautrix-signal"
    exit 1
}
...
echo ""
echo -e "${GR}mautrix-signal deployed on ${var_instance} (${IP})${NC}"
echo ""
