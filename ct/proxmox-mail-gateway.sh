#!/usr/bin/env bash
# ct/proxmox-mail-gateway.sh — Proxmox Mail Gateway
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/proxmox-mail-gateway-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for proxmox-mail-gateway"
    exit 1
}
...
echo ""
echo -e "${GR}Proxmox Mail Gateway deployed on ${var_instance} (${IP})${NC}"
echo ""
