#!/usr/bin/env bash
# ct/unifi-os-server.sh — Unifi Os Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/unifi-os-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for unifi-os-server"
    exit 1
}
...
echo ""
echo -e "${GR}Unifi Os Server deployed on ${var_instance} (${IP})${NC}"
echo ""
