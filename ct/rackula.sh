...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/rackula-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for rackula"
    exit 1
}
...
echo ""
echo -e "${GR}Rackula deployed on ${var_instance} (${IP})${NC}"
echo ""
