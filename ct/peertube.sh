...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/peertube-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for peertube"
    exit 1
}
...
echo ""
echo -e "${GR}Peertube deployed on ${var_instance} (${IP})${NC}"
echo ""
