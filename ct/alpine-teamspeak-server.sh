#!/usr/bin/env bash
# ct/alpine-teamspeak-server.sh — Alpine Teamspeak Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-teamspeak-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-teamspeak-server"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Teamspeak Server deployed on ${var_instance} (${IP})${NC}"
echo ""
