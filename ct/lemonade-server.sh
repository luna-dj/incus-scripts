#!/usr/bin/env bash
# ct/lemonade-server.sh — Lemonade Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/lemonade-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for lemonade-server"
    exit 1
}
...
echo ""
echo -e "${GR}Lemonade Server deployed on ${var_instance} (${IP})${NC}"
echo ""
