...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/autobrr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for autobrr"
    exit 1
}
...
echo ""
echo -e "${GR}Autobrr deployed on ${var_instance} (${IP})${NC}"
echo ""
