#!/usr/bin/env bash
# ct/urbackupserver.sh — Urbackupserver
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/urbackupserver-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for urbackupserver"
    exit 1
}
...
echo ""
echo -e "${GR}Urbackupserver deployed on ${var_instance} (${IP})${NC}"
echo ""
