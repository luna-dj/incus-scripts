...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/seanime-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for seanime"
    exit 1
}
...
echo ""
echo -e "${GR}Seanime deployed on ${var_instance} (${IP})${NC}"
echo ""
