#!/usr/bin/env bash
# ct/mail-archiver.sh — Mail Archiver
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/mail-archiver-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mail-archiver"
    exit 1
}
...
echo ""
echo -e "${GR}Mail Archiver deployed on ${var_instance} (${IP})${NC}"
echo ""
