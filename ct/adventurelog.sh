#!/usr/bin/env bash
# ct/adventurelog.sh — Adventurelog
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/adventurelog-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for adventurelog"
    exit 1
}
...
echo ""
echo -e "${GR}Adventurelog deployed on ${var_instance} (${IP})${NC}"
echo ""
