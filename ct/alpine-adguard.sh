#!/usr/bin/env bash
# ct/alpine-adguard.sh — Alpine Adguard
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-adguard-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-adguard"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Adguard deployed on ${var_instance} (${IP})${NC}"
echo ""
