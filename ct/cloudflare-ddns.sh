#!/usr/bin/env bash
# ct/cloudflare-ddns.sh — Cloudflare Ddns
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/cloudflare-ddns-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for cloudflare-ddns"
    exit 1
}
...
echo ""
echo -e "${GR}Cloudflare Ddns deployed on ${var_instance} (${IP})${NC}"
echo ""
