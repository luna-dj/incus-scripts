#!/usr/bin/env bash
# ct/gitea-mirror.sh — Gitea Mirror
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/gitea-mirror-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for gitea-mirror"
    exit 1
}
...
echo ""
echo -e "${GR}Gitea Mirror deployed on ${var_instance} (${IP})${NC}"
echo ""
