...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/fleet-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for fleet"
    exit 1
}
...
echo ""
echo -e "${GR}Fleet deployed on ${var_instance} (${IP})${NC}"
echo ""
