#!/usr/bin/env bash
# ct/proxmox-backup-server.sh — Proxmox Backup Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/proxmox-backup-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for proxmox-backup-server"
    exit 1
}
...
echo ""
echo -e "${GR}Proxmox Backup Server deployed on ${var_instance} (${IP})${NC}"
echo ""
