#!/usr/bin/env bash
# ct/alpine-docker.sh — Alpine Docker
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-docker-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-docker"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Docker deployed on ${var_instance} (${IP})${NC}"
echo ""
