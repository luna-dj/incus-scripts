#!/usr/bin/env bash
# ct/invoiceshelf.sh — Invoiceshelf
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/invoiceshelf-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for invoiceshelf"
    exit 1
}
...
echo ""
echo -e "${GR}Invoiceshelf deployed on ${var_instance} (${IP})${NC}"
echo ""
