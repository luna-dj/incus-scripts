#!/usr/bin/env bash
# ct/apache-airflow.sh — Apache Airflow
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/apache-airflow-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for apache-airflow"
    exit 1
}
...
echo ""
echo -e "${GR}Apache Airflow deployed on ${var_instance} (${IP})${NC}"
echo ""
