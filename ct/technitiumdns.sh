#!/usr/bin/env bash
# ct/technitiumdns.sh — Technitiumdns
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/technitiumdns-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for technitiumdns"
    exit 1
}
...
echo ""
echo -e "${GR}Technitiumdns deployed on ${var_instance} (${IP})${NC}"
echo ""
