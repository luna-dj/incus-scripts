#!/usr/bin/env bash
# ct/libretranslate.sh — Libretranslate
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/libretranslate-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for libretranslate"
    exit 1
}
...
echo ""
echo -e "${GR}Libretranslate deployed on ${var_instance} (${IP})${NC}"
echo ""
