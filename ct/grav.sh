...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/grav-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for grav"
    exit 1
}
...
echo ""
echo -e "${GR}Grav deployed on ${var_instance} (${IP})${NC}"
echo ""
