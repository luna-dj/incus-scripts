#!/usr/bin/env bash
# ct/alpine-wakapi.sh — Alpine Wakapi
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-wakapi-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-wakapi"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Wakapi deployed on ${var_instance} (${IP})${NC}"
echo ""
