#!/usr/bin/env bash
# ct/paperless-ai.sh — Paperless Ai
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/paperless-ai-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for paperless-ai"
    exit 1
}
...
echo ""
echo -e "${GR}Paperless Ai deployed on ${var_instance} (${IP})${NC}"
echo ""
