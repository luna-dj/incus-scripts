#!/usr/bin/env bash
# ct/zerotier-one.sh — Zerotier One
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/zerotier-one-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for zerotier-one"
    exit 1
}
...
echo ""
echo -e "${GR}Zerotier One deployed on ${var_instance} (${IP})${NC}"
echo ""
