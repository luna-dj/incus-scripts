#!/usr/bin/env bash
# ct/alpine-redis.sh — Alpine Redis
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-redis-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-redis"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Redis deployed on ${var_instance} (${IP})${NC}"
echo ""
