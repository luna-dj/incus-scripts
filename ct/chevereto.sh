...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/chevereto-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for chevereto"
    exit 1
}
...
echo ""
echo -e "${GR}Chevereto deployed on ${var_instance} (${IP})${NC}"
echo ""
