...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/omnitools-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for omnitools"
    exit 1
}
...
echo ""
echo -e "${GR}Omnitools deployed on ${var_instance} (${IP})${NC}"
echo ""
