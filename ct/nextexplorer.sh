#!/usr/bin/env bash
# ct/nextexplorer.sh — Nextexplorer
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/nextexplorer-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for nextexplorer"
    exit 1
}
...
echo ""
echo -e "${GR}Nextexplorer deployed on ${var_instance} (${IP})${NC}"
echo ""
