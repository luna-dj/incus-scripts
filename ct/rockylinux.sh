...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/rockylinux-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for rockylinux"
    exit 1
}
...
echo ""
echo -e "${GR}Rockylinux deployed on ${var_instance} (${IP})${NC}"
echo ""
