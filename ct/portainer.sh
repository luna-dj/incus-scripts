...
#!/usr/bin/env bash
# ct/portainer.sh — Portainer (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/portainer-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for portainer"
    exit 1
}
printf '%s
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Portainer deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Portainer install did not start a Docker container on ${var_instance} (${IP}).${NC}"
