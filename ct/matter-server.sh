#!/usr/bin/env bash
# ct/matter-server.sh — Matter Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/matter-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for matter-server"
    exit 1
}
...
echo ""
echo -e "${GR}Matter Server deployed on ${var_instance} (${IP})${NC}"
echo ""
