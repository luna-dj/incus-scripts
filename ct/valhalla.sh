...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/valhalla-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for valhalla"
    exit 1
}
...
echo ""
echo -e "${GR}Valhalla deployed on ${var_instance} (${IP})${NC}"
echo ""
