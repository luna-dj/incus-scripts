...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/securo-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for securo"
    exit 1
}
...
echo ""
echo -e "${GR}Securo deployed on ${var_instance} (${IP})${NC}"
echo ""
