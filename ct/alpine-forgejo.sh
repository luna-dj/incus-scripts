#!/usr/bin/env bash
# ct/alpine-forgejo.sh — Alpine Forgejo
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-forgejo-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-forgejo"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Forgejo deployed on ${var_instance} (${IP})${NC}"
echo ""
