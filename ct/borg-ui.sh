...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/borg-ui-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for borg-ui"
    exit 1
}
...
echo ""
echo -e "${GR}Borg Ui deployed on ${var_instance} (${IP})${NC}"
echo ""
