#!/usr/bin/env bash
# ct/alpine-wireguard.sh — Alpine Wireguard
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-wireguard-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-wireguard"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Wireguard deployed on ${var_instance} (${IP})${NC}"
echo ""
