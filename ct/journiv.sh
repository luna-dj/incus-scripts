...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/journiv-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for journiv"
    exit 1
}
...
echo ""
echo -e "${GR}Journiv deployed on ${var_instance} (${IP})${NC}"
echo ""
