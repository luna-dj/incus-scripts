#!/usr/bin/env bash
# ct/watchyourlan.sh — Watchyourlan
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/watchyourlan-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for watchyourlan"
    exit 1
}
...
echo ""
echo -e "${GR}Watchyourlan deployed on ${var_instance} (${IP})${NC}"
echo ""
