...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/node-red-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for node-red"
    exit 1
}
...
echo ""
echo -e "${GR}Node Red deployed on ${var_instance} (${IP})${NC}"
echo ""
