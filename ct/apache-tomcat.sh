#!/usr/bin/env bash
# ct/apache-tomcat.sh — Apache Tomcat
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/apache-tomcat-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for apache-tomcat"
    exit 1
}
...
echo ""
echo -e "${GR}Apache Tomcat deployed on ${var_instance} (${IP})${NC}"
echo ""
