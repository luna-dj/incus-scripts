#!/usr/bin/env bash
# ct/alpine-bitmagnet.sh — Alpine Bitmagnet
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-bitmagnet-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-bitmagnet"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Bitmagnet deployed on ${var_instance} (${IP})${NC}"
echo ""
