#!/usr/bin/env bash
# ct/joplin-server.sh — Joplin Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/joplin-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for joplin-server"
    exit 1
}
...
echo ""
echo -e "${GR}Joplin Server deployed on ${var_instance} (${IP})${NC}"
echo ""
