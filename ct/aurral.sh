...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/aurral-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for aurral"
    exit 1
}
...
echo ""
echo -e "${GR}Aurral deployed on ${var_instance} (${IP})${NC}"
echo ""
