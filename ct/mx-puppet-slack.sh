#!/usr/bin/env bash
# ct/mx-puppet-slack.sh — mx-puppet-slack (alt)
# Matrix bridge to Slack (alt bridge).
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mx-puppet-slack.sh)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mx-puppet-slack-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mx-puppet-slack"
    exit 1
}
...
echo ""
echo -e "${GR}mx-puppet-slack (alt) deployed on ${var_instance} (${IP})${NC}"
echo ""
