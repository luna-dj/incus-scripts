#!/usr/bin/env bash
# ct/satisfactory.sh — Satisfactory
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/satisfactory-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for satisfactory"
    exit 1
}
...
echo ""
echo -e "${GR}Satisfactory deployed on ${var_instance} (${IP})${NC}"
echo ""
