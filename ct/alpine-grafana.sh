#!/usr/bin/env bash
# ct/alpine-grafana.sh — Alpine Grafana
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-grafana-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-grafana"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Grafana deployed on ${var_instance} (${IP})${NC}"
echo ""
