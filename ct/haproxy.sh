...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/haproxy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for haproxy"
    exit 1
}
...
echo ""
echo -e "${GR}Haproxy deployed on ${var_instance} (${IP})${NC}"
echo ""
