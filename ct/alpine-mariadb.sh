#!/usr/bin/env bash
# ct/alpine-mariadb.sh — Alpine Mariadb
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-mariadb-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-mariadb"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Mariadb deployed on ${var_instance} (${IP})${NC}"
echo ""
