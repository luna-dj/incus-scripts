#!/usr/bin/env bash
# ct/mautrix-googlechat.sh — mautrix-googlechat
# Matrix bridge to Google Chat.
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mautrix-googlechat.sh)
...
APP="mautrix-googlechat"
var_tags="${var_tags:-matrix;bridge;googlechat}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-512}"
var_disk="${var_disk:-4}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mautrix-googlechat-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mautrix-googlechat"
    exit 1
}
...
echo ""
echo -e "${GR}mautrix-googlechat deployed on ${var_instance} (${IP})${NC}"
echo ""
