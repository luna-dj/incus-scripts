...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/ignis-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for ignis"
    exit 1
}
...
echo ""
echo -e "${GR}Ignis deployed on ${var_instance} (${IP})${NC}"
echo ""
