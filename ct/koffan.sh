...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/koffan-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for koffan"
    exit 1
}
...
echo ""
echo -e "${GR}Koffan deployed on ${var_instance} (${IP})${NC}"
echo ""
