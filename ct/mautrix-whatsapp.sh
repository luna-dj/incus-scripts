#!/usr/bin/env bash
# ct/mautrix-whatsapp.sh — mautrix-whatsapp
# Matrix bridge to WhatsApp.
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mautrix-whatsapp.sh)
...
var_tags="${var_tags:-matrix;bridge;whatsapp}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-768}"
var_disk="${var_disk:-6}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mautrix-whatsapp-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mautrix-whatsapp"
    exit 1
}
...
echo ""
echo -e "${GR}mautrix-whatsapp deployed on ${var_instance} (${IP})${NC}"
echo ""
