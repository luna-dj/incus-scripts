...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/shiori-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for shiori"
    exit 1
}
...
echo ""
echo -e "${GR}Shiori deployed on ${var_instance} (${IP})${NC}"
echo ""
