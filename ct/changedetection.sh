#!/usr/bin/env bash
# ct/changedetection.sh — Changedetection
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/changedetection-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for changedetection"
    exit 1
}
...
echo ""
echo -e "${GR}Changedetection deployed on ${var_instance} (${IP})${NC}"
echo ""
