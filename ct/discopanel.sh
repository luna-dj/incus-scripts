...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/discopanel-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for discopanel"
    exit 1
}
...
echo ""
echo -e "${GR}Discopanel deployed on ${var_instance} (${IP})${NC}"
echo ""
