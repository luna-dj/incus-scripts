...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/yopass-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for yopass"
    exit 1
}
...
echo ""
echo -e "${GR}Yopass deployed on ${var_instance} (${IP})${NC}"
echo ""
