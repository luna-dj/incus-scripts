...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/seerr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for seerr"
    exit 1
}
...
echo ""
echo -e "${GR}Seerr deployed on ${var_instance} (${IP})${NC}"
echo ""
