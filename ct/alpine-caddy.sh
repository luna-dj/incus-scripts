#!/usr/bin/env bash
# ct/alpine-caddy.sh — Alpine Caddy
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-caddy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-caddy"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Caddy deployed on ${var_instance} (${IP})${NC}"
echo ""
