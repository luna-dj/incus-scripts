#!/usr/bin/env bash
# ct/alpine-zigbee2mqtt.sh — Alpine Zigbee2Mqtt
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-zigbee2mqtt-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-zigbee2mqtt"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Zigbee2Mqtt deployed on ${var_instance} (${IP})${NC}"
echo ""
