#!/usr/bin/env bash
# ct/forgejo-runner.sh — Forgejo Runner
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/forgejo-runner-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for forgejo-runner"
    exit 1
}
...
echo ""
echo -e "${GR}Forgejo Runner deployed on ${var_instance} (${IP})${NC}"
echo ""
