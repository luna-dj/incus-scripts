#!/usr/bin/env bash
# ct/alpine-gatus.sh — Alpine Gatus
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-gatus-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-gatus"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Gatus deployed on ${var_instance} (${IP})${NC}"
echo ""
