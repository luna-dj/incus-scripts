#!/usr/bin/env bash
# ct/alpine-it-tools.sh — Alpine It Tools
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-it-tools-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-it-tools"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine It Tools deployed on ${var_instance} (${IP})${NC}"
echo ""
