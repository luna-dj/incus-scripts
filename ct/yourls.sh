...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/yourls-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for yourls"
    exit 1
}
...
echo ""
echo -e "${GR}Yourls deployed on ${var_instance} (${IP})${NC}"
echo ""
