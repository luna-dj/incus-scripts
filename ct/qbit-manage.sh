...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/qbit-manage-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for qbit-manage"
    exit 1
}
...
echo ""
echo -e "${GR}Qbit Manage deployed on ${var_instance} (${IP})${NC}"
echo ""
