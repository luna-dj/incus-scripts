#!/usr/bin/env bash
# ct/pelican-wings.sh — Pelican Wings
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/pelican-wings-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pelican-wings"
    exit 1
}
...
echo ""
echo -e "${GR}Pelican Wings deployed on ${var_instance} (${IP})${NC}"
echo ""
