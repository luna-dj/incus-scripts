...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/snapotter-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for snapotter"
    exit 1
}
...
echo ""
echo -e "${GR}Snapotter deployed on ${var_instance} (${IP})${NC}"
echo ""
