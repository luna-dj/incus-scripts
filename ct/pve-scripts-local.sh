#!/usr/bin/env bash
# ct/pve-scripts-local.sh — Pve Scripts Local
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/pve-scripts-local-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pve-scripts-local"
    exit 1
}
...
echo ""
echo -e "${GR}Pve Scripts Local deployed on ${var_instance} (${IP})${NC}"
echo ""
