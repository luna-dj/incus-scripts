#!/usr/bin/env bash
# ct/openthread-br.sh — Openthread Br
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/openthread-br-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for openthread-br"
    exit 1
}
...
echo ""
echo -e "${GR}Openthread Br deployed on ${var_instance} (${IP})${NC}"
echo ""
