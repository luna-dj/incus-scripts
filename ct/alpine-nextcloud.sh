#!/usr/bin/env bash
# ct/alpine-nextcloud.sh — Alpine Nextcloud
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-nextcloud-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-nextcloud"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Nextcloud deployed on ${var_instance} (${IP})${NC}"
echo ""
