#!/usr/bin/env bash
# ct/hev-socks5-server.sh — Hev Socks5 Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/hev-socks5-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for hev-socks5-server"
    exit 1
}
...
echo ""
echo -e "${GR}Hev Socks5 Server deployed on ${var_instance} (${IP})${NC}"
echo ""
