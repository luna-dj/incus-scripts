...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine deployed on ${var_instance} (${IP})${NC}"
echo ""
