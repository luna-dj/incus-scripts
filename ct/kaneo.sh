...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/kaneo-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for kaneo"
    exit 1
}
...
echo ""
echo -e "${GR}Kaneo deployed on ${var_instance} (${IP})${NC}"
echo ""
