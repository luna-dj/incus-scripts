#!/usr/bin/env bash
# ct/alpine-ironclaw.sh — Alpine Ironclaw
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-ironclaw-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-ironclaw"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Ironclaw deployed on ${var_instance} (${IP})${NC}"
echo ""
