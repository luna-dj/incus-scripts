...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/portabase-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for portabase"
    exit 1
}
...
echo ""
echo -e "${GR}Portabase deployed on ${var_instance} (${IP})${NC}"
echo ""
