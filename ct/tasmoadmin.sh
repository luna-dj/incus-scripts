...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/tasmoadmin-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for tasmoadmin"
    exit 1
}
...
echo ""
echo -e "${GR}Tasmoadmin deployed on ${var_instance} (${IP})${NC}"
echo ""
