...
#!/usr/bin/env bash
# ct/all-templates.sh — All Templates (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/all-templates-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for all-templates"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "all-templates"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}All Templates deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}All Templates install did not start a Docker container on ${var_instance} (${IP}).${NC}"
