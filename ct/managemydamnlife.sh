#!/usr/bin/env bash
# ct/managemydamnlife.sh — Managemydamnlife
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/managemydamnlife-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for managemydamnlife"
    exit 1
}
...
echo ""
echo -e "${GR}Managemydamnlife deployed on ${var_instance} (${IP})${NC}"
echo ""
