#!/usr/bin/env bash
# ct/alpine-prometheus.sh — Alpine Prometheus
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-prometheus-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-prometheus"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Prometheus deployed on ${var_instance} (${IP})${NC}"
echo ""
