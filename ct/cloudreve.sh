...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/cloudreve-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for cloudreve"
    exit 1
}
...
echo ""
echo -e "${GR}Cloudreve deployed on ${var_instance} (${IP})${NC}"
echo ""
