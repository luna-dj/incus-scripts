#!/usr/bin/env bash
# ct/alpine-garage.sh — Alpine Garage
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-garage-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-garage"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Garage deployed on ${var_instance} (${IP})${NC}"
echo ""
