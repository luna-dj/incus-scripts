...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/docspell-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for docspell"
    exit 1
}
...
echo ""
echo -e "${GR}Docspell deployed on ${var_instance} (${IP})${NC}"
echo ""
