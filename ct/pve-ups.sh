...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/pve-ups-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pve-ups"
    exit 1
}
...
echo ""
echo -e "${GR}Pve Ups deployed on ${var_instance} (${IP})${NC}"
echo ""
