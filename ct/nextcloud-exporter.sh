...
#!/usr/bin/env bash
# ct/nextcloud-exporter.sh — Nextcloud Exporter (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/nextcloud-exporter-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for nextcloud-exporter"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "nextcloud-exporter"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Nextcloud Exporter deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Nextcloud Exporter install did not start a Docker container on ${var_instance} (${IP}).${NC}"
