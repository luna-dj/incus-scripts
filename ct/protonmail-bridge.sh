#!/usr/bin/env bash
# ct/protonmail-bridge.sh — Protonmail Bridge
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/protonmail-bridge-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for protonmail-bridge"
    exit 1
}
...
echo ""
echo -e "${GR}Protonmail Bridge deployed on ${var_instance} (${IP})${NC}"
echo ""
