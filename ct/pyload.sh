...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/pyload-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pyload"
    exit 1
}
...
echo ""
echo -e "${GR}Pyload deployed on ${var_instance} (${IP})${NC}"
echo ""
