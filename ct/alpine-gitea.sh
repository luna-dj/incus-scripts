#!/usr/bin/env bash
# ct/alpine-gitea.sh — Alpine Gitea
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-gitea-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-gitea"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Gitea deployed on ${var_instance} (${IP})${NC}"
echo ""
