...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/stash-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for stash"
    exit 1
}
...
echo ""
echo -e "${GR}Stash deployed on ${var_instance} (${IP})${NC}"
echo ""
