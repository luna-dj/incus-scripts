...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/netboot-xyz-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for netboot-xyz"
    exit 1
}
...
echo ""
echo -e "${GR}Netboot Xyz deployed on ${var_instance} (${IP})${NC}"
echo ""
