...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/spoolman-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for spoolman"
    exit 1
}
...
echo ""
echo -e "${GR}Spoolman deployed on ${var_instance} (${IP})${NC}"
echo ""
