#!/usr/bin/env bash
# ct/domain-locker.sh — Domain Locker
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/domain-locker-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for domain-locker"
    exit 1
}
...
echo ""
echo -e "${GR}Domain Locker deployed on ${var_instance} (${IP})${NC}"
echo ""
