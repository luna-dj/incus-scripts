#!/usr/bin/env bash
# ct/prometheus-pve-exporter.sh — Prometheus Pve Exporter
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/prometheus-pve-exporter-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for prometheus-pve-exporter"
    exit 1
}
...
echo ""
echo -e "${GR}Prometheus Pve Exporter deployed on ${var_instance} (${IP})${NC}"
echo ""
