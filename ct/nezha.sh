...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/nezha-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for nezha"
    exit 1
}
...
echo ""
echo -e "${GR}Nezha deployed on ${var_instance} (${IP})${NC}"
echo ""
