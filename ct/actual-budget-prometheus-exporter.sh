...
#!/usr/bin/env bash
# ct/actual-budget-prometheus-exporter.sh — Actual Budget Prometheus Exporter (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/actual-budget-prometheus-exporter-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for actual-budget-prometheus-exporter"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "actual-budget-prometheus-exporter"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Actual Budget Prometheus Exporter deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Actual Budget Prometheus Exporter install did not start a Docker container on ${var_instance} (${IP}).${NC}"
