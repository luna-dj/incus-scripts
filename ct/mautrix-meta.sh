#!/usr/bin/env bash
# ct/mautrix-meta.sh — mautrix-meta (Messenger + Instagram)
# Matrix bridge to Messenger/Instagram.
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mautrix-meta.sh)
...
APP="mautrix-meta (Messenger + Instagram)"
var_tags="${var_tags:-matrix;bridge;meta}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-768}"
var_disk="${var_disk:-6}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mautrix-meta-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mautrix-meta"
    exit 1
}
...
echo ""
echo -e "${GR}mautrix-meta (Messenger + Instagram) deployed on ${var_instance} (${IP})${NC}"
echo ""
