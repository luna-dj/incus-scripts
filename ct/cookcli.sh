...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/cookcli-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for cookcli"
    exit 1
}
...
echo ""
echo -e "${GR}Cookcli deployed on ${var_instance} (${IP})${NC}"
echo ""
