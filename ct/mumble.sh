...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mumble-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mumble"
    exit 1
}
...
echo ""
echo -e "${GR}Mumble deployed on ${var_instance} (${IP})${NC}"
echo ""
