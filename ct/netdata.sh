...
#!/usr/bin/env bash
# ct/netdata.sh — Netdata (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/netdata-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for netdata"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "netdata"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Netdata deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Netdata install did not start a Docker container on ${var_instance} (${IP}).${NC}"
