...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/rss-bridge-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for rss-bridge"
    exit 1
}
...
echo ""
echo -e "${GR}Rss Bridge deployed on ${var_instance} (${IP})${NC}"
echo ""
