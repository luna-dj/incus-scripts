...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/iventoy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for iventoy"
    exit 1
}
...
echo ""
echo -e "${GR}Iventoy deployed on ${var_instance} (${IP})${NC}"
echo ""
