#!/usr/bin/env bash
# ct/umlautadaptarr.sh — Umlautadaptarr
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/umlautadaptarr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for umlautadaptarr"
    exit 1
}
...
echo ""
echo -e "${GR}Umlautadaptarr deployed on ${var_instance} (${IP})${NC}"
echo ""
