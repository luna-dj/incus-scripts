#!/usr/bin/env bash
# ct/librespeed-rust.sh — Librespeed Rust
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/librespeed-rust-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for librespeed-rust"
    exit 1
}
...
echo ""
echo -e "${GR}Librespeed Rust deployed on ${var_instance} (${IP})${NC}"
echo ""
