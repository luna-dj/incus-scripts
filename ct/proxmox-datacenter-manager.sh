#!/usr/bin/env bash
# ct/proxmox-datacenter-manager.sh — Proxmox Datacenter Manager
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/proxmox-datacenter-manager-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for proxmox-datacenter-manager"
    exit 1
}
...
echo ""
echo -e "${GR}Proxmox Datacenter Manager deployed on ${var_instance} (${IP})${NC}"
echo ""
