#!/usr/bin/env bash
# ct/languagetool.sh — Languagetool
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/languagetool-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for languagetool"
    exit 1
}
...
echo ""
echo -e "${GR}Languagetool deployed on ${var_instance} (${IP})${NC}"
echo ""
