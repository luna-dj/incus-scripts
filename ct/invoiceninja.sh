#!/usr/bin/env bash
# ct/invoiceninja.sh — Invoiceninja
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/invoiceninja-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for invoiceninja"
    exit 1
}
...
echo ""
echo -e "${GR}Invoiceninja deployed on ${var_instance} (${IP})${NC}"
echo ""
