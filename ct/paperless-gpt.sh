#!/usr/bin/env bash
# ct/paperless-gpt.sh — Paperless Gpt
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/paperless-gpt-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for paperless-gpt"
    exit 1
}
...
echo ""
echo -e "${GR}Paperless Gpt deployed on ${var_instance} (${IP})${NC}"
echo ""
