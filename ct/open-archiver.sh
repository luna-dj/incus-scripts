#!/usr/bin/env bash
# ct/open-archiver.sh — Open Archiver
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/open-archiver-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for open-archiver"
    exit 1
}
...
echo ""
echo -e "${GR}Open Archiver deployed on ${var_instance} (${IP})${NC}"
echo ""
