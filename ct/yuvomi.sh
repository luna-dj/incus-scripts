...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/yuvomi-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for yuvomi"
    exit 1
}
...
echo ""
echo -e "${GR}Yuvomi deployed on ${var_instance} (${IP})${NC}"
echo ""
