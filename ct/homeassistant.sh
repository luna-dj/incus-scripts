#!/usr/bin/env bash
# ct/homeassistant.sh — Homeassistant
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/homeassistant-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for homeassistant"
    exit 1
}
...
echo ""
echo -e "${GR}Homeassistant deployed on ${var_instance} (${IP})${NC}"
echo ""
