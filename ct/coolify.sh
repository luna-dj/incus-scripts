...
#!/usr/bin/env bash
# ct/coolify.sh — Coolify (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/coolify-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for coolify"
    exit 1
}
printf '%s
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Coolify deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Coolify install did not start a Docker container on ${var_instance} (${IP}).${NC}"
