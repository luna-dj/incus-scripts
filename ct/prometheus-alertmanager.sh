#!/usr/bin/env bash
# ct/prometheus-alertmanager.sh — Prometheus Alertmanager
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/prometheus-alertmanager-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for prometheus-alertmanager"
    exit 1
}
...
echo ""
echo -e "${GR}Prometheus Alertmanager deployed on ${var_instance} (${IP})${NC}"
echo ""
