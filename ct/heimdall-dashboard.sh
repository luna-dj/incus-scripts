#!/usr/bin/env bash
# ct/heimdall-dashboard.sh — Heimdall Dashboard
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/heimdall-dashboard-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for heimdall-dashboard"
    exit 1
}
...
echo ""
echo -e "${GR}Heimdall Dashboard deployed on ${var_instance} (${IP})${NC}"
echo ""
