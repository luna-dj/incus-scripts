#!/usr/bin/env bash
# ct/pterodactyl-panel.sh — Pterodactyl Panel
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/pterodactyl-panel-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pterodactyl-panel"
    exit 1
}
...
echo ""
echo -e "${GR}Pterodactyl Panel deployed on ${var_instance} (${IP})${NC}"
echo ""
