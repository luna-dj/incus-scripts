#!/usr/bin/env bash
# ct/alpine-rustypaste.sh — Alpine Rustypaste
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-rustypaste-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-rustypaste"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Rustypaste deployed on ${var_instance} (${IP})${NC}"
echo ""
