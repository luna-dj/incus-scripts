#!/usr/bin/env bash
# ct/alpine-node-red.sh — Alpine Node Red
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-node-red-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-node-red"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Node Red deployed on ${var_instance} (${IP})${NC}"
echo ""
