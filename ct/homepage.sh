...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/homepage-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for homepage"
    exit 1
}
...
echo ""
echo -e "${GR}Homepage deployed on ${var_instance} (${IP})${NC}"
echo ""
