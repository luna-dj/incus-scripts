#!/usr/bin/env bash
# ct/mautrix-imessage.sh — mautrix-imessage (macOS only)
# Matrix bridge to iMessage.
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/mautrix-imessage.sh)
...
APP="mautrix-imessage (macOS only)"
var_tags="${var_tags:-matrix;bridge;imessage}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-512}"
var_disk="${var_disk:-4}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mautrix-imessage-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mautrix-imessage"
    exit 1
}
...
echo ""
echo -e "${GR}mautrix-imessage (macOS only) deployed on ${var_instance} (${IP})${NC}"
echo ""
