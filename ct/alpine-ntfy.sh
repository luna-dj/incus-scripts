...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-ntfy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-ntfy"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Ntfy deployed on ${var_instance} (${IP})${NC}"
echo ""
