#!/usr/bin/env bash
# ct/super-productivity.sh — Super Productivity
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/super-productivity-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for super-productivity"
    exit 1
}
...
echo ""
echo -e "${GR}Super Productivity deployed on ${var_instance} (${IP})${NC}"
echo ""
