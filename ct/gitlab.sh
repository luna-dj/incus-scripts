...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/gitlab-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for gitlab"
    exit 1
}
...
echo ""
echo -e "${GR}Gitlab deployed on ${var_instance} (${IP})${NC}"
echo ""
