...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/stalwart-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for stalwart"
    exit 1
}
...
echo ""
echo -e "${GR}Stalwart deployed on ${var_instance} (${IP})${NC}"
echo ""
