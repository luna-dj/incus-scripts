...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/leafwiki-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for leafwiki"
    exit 1
}
...
echo ""
echo -e "${GR}Leafwiki deployed on ${var_instance} (${IP})${NC}"
echo ""
