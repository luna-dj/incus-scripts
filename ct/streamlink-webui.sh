#!/usr/bin/env bash
# ct/streamlink-webui.sh — Streamlink Webui
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/streamlink-webui-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for streamlink-webui"
    exit 1
}
...
echo ""
echo -e "${GR}Streamlink Webui deployed on ${var_instance} (${IP})${NC}"
echo ""
