...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/reactive-resume-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for reactive-resume"
    exit 1
}
