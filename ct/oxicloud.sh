...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/oxicloud-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for oxicloud"
    exit 1
}
...
echo ""
echo -e "${GR}Oxicloud deployed on ${var_instance} (${IP})${NC}"
echo ""
