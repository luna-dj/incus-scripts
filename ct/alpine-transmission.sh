#!/usr/bin/env bash
# ct/alpine-transmission.sh — Alpine Transmission
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-transmission-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-transmission"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Transmission deployed on ${var_instance} (${IP})${NC}"
echo ""
