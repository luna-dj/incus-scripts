#!/usr/bin/env bash
# ct/alpine-valkey.sh — Alpine Valkey
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-valkey-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-valkey"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Valkey deployed on ${var_instance} (${IP})${NC}"
echo ""
