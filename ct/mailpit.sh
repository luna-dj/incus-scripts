...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mailpit-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mailpit"
    exit 1
}
...
echo ""
echo -e "${GR}Mailpit deployed on ${var_instance} (${IP})${NC}"
echo ""
