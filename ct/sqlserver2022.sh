#!/usr/bin/env bash
# ct/sqlserver2022.sh — Sqlserver2022
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/sqlserver2022-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for sqlserver2022"
    exit 1
}
...
echo ""
echo -e "${GR}Sqlserver2022 deployed on ${var_instance} (${IP})${NC}"
echo ""
