...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/centos-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for centos"
    exit 1
}
...
echo ""
echo -e "${GR}Centos deployed on ${var_instance} (${IP})${NC}"
echo ""
