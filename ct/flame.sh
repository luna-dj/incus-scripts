...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/flame-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for flame"
    exit 1
}
...
echo ""
echo -e "${GR}Flame deployed on ${var_instance} (${IP})${NC}"
echo ""
