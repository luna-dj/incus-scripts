#!/usr/bin/env bash
# ct/appservice-irc.sh — matrix-appservice-irc
# Matrix bridge to IRC (legacy).
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/appservice-irc.sh)
...
var_tags="${var_tags:-matrix;bridge;irc}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-256}"
var_disk="${var_disk:-2}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/appservice-irc-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for appservice-irc"
    exit 1
}
...
echo ""
echo -e "${GR}matrix-appservice-irc deployed on ${var_instance} (${IP})${NC}"
echo ""
