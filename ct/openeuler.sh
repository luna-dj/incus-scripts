...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/openeuler-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for openeuler"
    exit 1
}
...
echo ""
echo -e "${GR}Openeuler deployed on ${var_instance} (${IP})${NC}"
echo ""
