...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/fedora-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for fedora"
    exit 1
}
...
echo ""
echo -e "${GR}Fedora deployed on ${var_instance} (${IP})${NC}"
echo ""
