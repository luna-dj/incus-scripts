...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/orb-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for orb"
    exit 1
}
...
echo ""
echo -e "${GR}Orb deployed on ${var_instance} (${IP})${NC}"
echo ""
