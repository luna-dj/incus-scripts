#!/usr/bin/env bash
# ct/prometheus-blackbox-exporter.sh — Prometheus Blackbox Exporter
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/prometheus-blackbox-exporter-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for prometheus-blackbox-exporter"
    exit 1
}
...
echo ""
echo -e "${GR}Prometheus Blackbox Exporter deployed on ${var_instance} (${IP})${NC}"
echo ""
