#!/usr/bin/env bash
# ct/lazylibrarian.sh — Lazylibrarian
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/lazylibrarian-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for lazylibrarian"
    exit 1
}
...
echo ""
echo -e "${GR}Lazylibrarian deployed on ${var_instance} (${IP})${NC}"
echo ""
