#!/usr/bin/env bash
# ct/beaverhabits.sh — Beaverhabits
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/beaverhabits-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for beaverhabits"
    exit 1
}
...
echo ""
echo -e "${GR}Beaverhabits deployed on ${var_instance} (${IP})${NC}"
echo ""
