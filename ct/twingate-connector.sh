#!/usr/bin/env bash
# ct/twingate-connector.sh — Twingate Connector
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/twingate-connector-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for twingate-connector"
    exit 1
}
...
echo ""
echo -e "${GR}Twingate Connector deployed on ${var_instance} (${IP})${NC}"
echo ""
