#!/usr/bin/env bash
# ct/lyrionmusicserver.sh — Lyrionmusicserver
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/lyrionmusicserver-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for lyrionmusicserver"
    exit 1
}
...
echo ""
echo -e "${GR}Lyrionmusicserver deployed on ${var_instance} (${IP})${NC}"
echo ""
