...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/newt-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for newt"
    exit 1
}
...
echo ""
echo -e "${GR}Newt deployed on ${var_instance} (${IP})${NC}"
echo ""
