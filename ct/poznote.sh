...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/poznote-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for poznote"
    exit 1
}
...
echo ""
echo -e "${GR}Poznote deployed on ${var_instance} (${IP})${NC}"
echo ""
