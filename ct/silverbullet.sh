#!/usr/bin/env bash
# ct/silverbullet.sh — Silverbullet
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/silverbullet-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for silverbullet"
    exit 1
}
...
echo ""
echo -e "${GR}Silverbullet deployed on ${var_instance} (${IP})${NC}"
echo ""
