#!/usr/bin/env bash
# ct/bitfocus-companion.sh — Bitfocus Companion
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/bitfocus-companion-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for bitfocus-companion"
    exit 1
}
...
echo ""
echo -e "${GR}Bitfocus Companion deployed on ${var_instance} (${IP})${NC}"
echo ""
