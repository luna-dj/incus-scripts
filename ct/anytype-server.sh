#!/usr/bin/env bash
# ct/anytype-server.sh — Anytype Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/anytype-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for anytype-server"
    exit 1
}
...
echo ""
echo -e "${GR}Anytype Server deployed on ${var_instance} (${IP})${NC}"
echo ""
