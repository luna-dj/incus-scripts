#!/usr/bin/env bash
# ct/ezbookkeeping.sh — Ezbookkeeping
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/ezbookkeeping-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for ezbookkeeping"
    exit 1
}
...
echo ""
echo -e "${GR}Ezbookkeeping deployed on ${var_instance} (${IP})${NC}"
echo ""
