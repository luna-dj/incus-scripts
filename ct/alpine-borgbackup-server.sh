#!/usr/bin/env bash
# ct/alpine-borgbackup-server.sh — Alpine Borgbackup Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-borgbackup-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-borgbackup-server"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Borgbackup Server deployed on ${var_instance} (${IP})${NC}"
echo ""
