#!/usr/bin/env bash
# ct/yt-dlp-webui.sh — Yt Dlp Webui
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/yt-dlp-webui-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for yt-dlp-webui"
    exit 1
}
...
echo ""
echo -e "${GR}Yt Dlp Webui deployed on ${var_instance} (${IP})${NC}"
echo ""
