...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/safebucket-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for safebucket"
    exit 1
}
...
echo ""
echo -e "${GR}Safebucket deployed on ${var_instance} (${IP})${NC}"
echo ""
