#!/usr/bin/env bash
# ct/solidinvoice.sh — Solidinvoice
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/solidinvoice-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for solidinvoice"
    exit 1
}
...
echo ""
echo -e "${GR}Solidinvoice deployed on ${var_instance} (${IP})${NC}"
echo ""
