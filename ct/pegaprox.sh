...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/pegaprox-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for pegaprox"
    exit 1
}
...
echo ""
echo -e "${GR}Pegaprox deployed on ${var_instance} (${IP})${NC}"
echo ""
