#!/usr/bin/env bash
# ct/fluid-calendar.sh — Fluid Calendar
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/fluid-calendar-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for fluid-calendar"
    exit 1
}
...
echo ""
echo -e "${GR}Fluid Calendar deployed on ${var_instance} (${IP})${NC}"
echo ""
