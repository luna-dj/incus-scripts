...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/openbao-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for openbao"
    exit 1
}
...
echo ""
echo -e "${GR}Openbao deployed on ${var_instance} (${IP})${NC}"
echo ""
