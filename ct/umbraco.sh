...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/umbraco-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for umbraco"
    exit 1
}
...
echo ""
echo -e "${GR}Umbraco deployed on ${var_instance} (${IP})${NC}"
echo ""
