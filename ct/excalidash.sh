...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/excalidash-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for excalidash"
    exit 1
}
...
echo ""
echo -e "${GR}Excalidash deployed on ${var_instance} (${IP})${NC}"
echo ""
