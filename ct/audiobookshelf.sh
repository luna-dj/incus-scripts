#!/usr/bin/env bash
# ct/audiobookshelf.sh — Audiobookshelf
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/audiobookshelf-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for audiobookshelf"
    exit 1
}
...
echo ""
echo -e "${GR}Audiobookshelf deployed on ${var_instance} (${IP})${NC}"
echo ""
