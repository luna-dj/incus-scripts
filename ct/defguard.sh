...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/defguard-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for defguard"
    exit 1
}
...
echo ""
echo -e "${GR}Defguard deployed on ${var_instance} (${IP})${NC}"
echo ""
