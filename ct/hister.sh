...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/hister-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for hister"
    exit 1
}
...
echo ""
echo -e "${GR}Hister deployed on ${var_instance} (${IP})${NC}"
echo ""
