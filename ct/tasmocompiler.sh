#!/usr/bin/env bash
# ct/tasmocompiler.sh — Tasmocompiler
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/tasmocompiler-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for tasmocompiler"
    exit 1
}
...
echo ""
echo -e "${GR}Tasmocompiler deployed on ${var_instance} (${IP})${NC}"
echo ""
