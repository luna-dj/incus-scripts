#!/usr/bin/env bash
# ct/your-spotify.sh — Your Spotify
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/your-spotify-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for your-spotify"
    exit 1
}
...
echo ""
echo -e "${GR}Your Spotify deployed on ${var_instance} (${IP})${NC}"
echo ""
