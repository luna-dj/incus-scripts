#!/usr/bin/env bash
# ct/sparkyfitness.sh — Sparkyfitness
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/sparkyfitness-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for sparkyfitness"
    exit 1
}
...
echo ""
echo -e "${GR}Sparkyfitness deployed on ${var_instance} (${IP})${NC}"
echo ""
