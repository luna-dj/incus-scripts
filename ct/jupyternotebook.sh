#!/usr/bin/env bash
# ct/jupyternotebook.sh — Jupyternotebook
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/jupyternotebook-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for jupyternotebook"
    exit 1
}
...
echo ""
echo -e "${GR}Jupyternotebook deployed on ${var_instance} (${IP})${NC}"
echo ""
