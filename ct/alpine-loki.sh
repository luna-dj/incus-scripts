...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-loki-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-loki"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Loki deployed on ${var_instance} (${IP})${NC}"
echo ""
