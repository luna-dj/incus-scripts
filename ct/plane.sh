...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/plane-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for plane"
    exit 1
}
...
echo ""
echo -e "${GR}Plane deployed on ${var_instance} (${IP})${NC}"
echo ""
