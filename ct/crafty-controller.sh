#!/usr/bin/env bash
# ct/crafty-controller.sh — Crafty Controller
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/crafty-controller-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for crafty-controller"
    exit 1
}
...
echo ""
echo -e "${GR}Crafty Controller deployed on ${var_instance} (${IP})${NC}"
echo ""
