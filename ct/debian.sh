...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/debian-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for debian"
    exit 1
}
...
echo ""
echo -e "${GR}Debian deployed on ${var_instance} (${IP})${NC}"
echo ""
