...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/postiz-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for postiz"
    exit 1
}
...
echo ""
echo -e "${GR}Postiz deployed on ${var_instance} (${IP})${NC}"
echo ""
