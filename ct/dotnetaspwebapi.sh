#!/usr/bin/env bash
# ct/dotnetaspwebapi.sh — Dotnetaspwebapi
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/dotnetaspwebapi-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for dotnetaspwebapi"
    exit 1
}
...
echo ""
echo -e "${GR}Dotnetaspwebapi deployed on ${var_instance} (${IP})${NC}"
echo ""
