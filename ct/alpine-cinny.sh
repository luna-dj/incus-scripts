#!/usr/bin/env bash
# ct/alpine-cinny.sh — Alpine Cinny
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-cinny-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-cinny"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Cinny deployed on ${var_instance} (${IP})${NC}"
echo ""
