#!/usr/bin/env bash
# ct/budget-board.sh — Budget Board
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/budget-board-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for budget-board"
    exit 1
}
...
echo ""
echo -e "${GR}Budget Board deployed on ${var_instance} (${IP})${NC}"
echo ""
