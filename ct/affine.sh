...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/affine-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for affine"
    exit 1
}
...
echo ""
echo -e "${GR}Affine deployed on ${var_instance} (${IP})${NC}"
echo ""
