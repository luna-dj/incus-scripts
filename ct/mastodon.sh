...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/mastodon-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mastodon"
    exit 1
}
...
echo ""
echo -e "${GR}Mastodon deployed on ${var_instance} (${IP})${NC}"
echo ""
