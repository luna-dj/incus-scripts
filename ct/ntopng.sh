...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/ntopng-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for ntopng"
    exit 1
}
...
echo ""
echo -e "${GR}Ntopng deployed on ${var_instance} (${IP})${NC}"
echo ""
