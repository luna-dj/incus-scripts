#!/usr/bin/env bash
# ct/matterjs-server.sh — Matterjs Server
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/matterjs-server-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for matterjs-server"
    exit 1
}
...
echo ""
echo -e "${GR}Matterjs Server deployed on ${var_instance} (${IP})${NC}"
echo ""
