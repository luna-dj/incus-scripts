...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/gentoo-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for gentoo"
    exit 1
}
...
echo ""
echo -e "${GR}Gentoo deployed on ${var_instance} (${IP})${NC}"
echo ""
