#!/usr/bin/env bash
# ct/mediamanager.sh — Mediamanager
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/mediamanager-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for mediamanager"
    exit 1
}
...
echo ""
echo -e "${GR}Mediamanager deployed on ${var_instance} (${IP})${NC}"
echo ""
