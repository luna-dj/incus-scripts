...
#!/usr/bin/env bash
# ct/olivetin.sh — OliveTin (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/olivetin-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for olivetin"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "olivetin"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}OliveTin deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}OliveTin install did not start a Docker container on ${var_instance} (${IP}).${NC}"
