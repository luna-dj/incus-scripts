...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/readmeabook-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for readmeabook"
    exit 1
}
...
echo ""
echo -e "${GR}Readmeabook deployed on ${var_instance} (${IP})${NC}"
echo ""
