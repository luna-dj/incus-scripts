...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/almalinux-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for almalinux"
    exit 1
}
...
echo ""
echo -e "${GR}Almalinux deployed on ${var_instance} (${IP})${NC}"
echo ""
