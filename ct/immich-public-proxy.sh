...
#!/usr/bin/env bash
# ct/immich-public-proxy.sh — Immich Public Proxy (addon, Docker-based)
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/immich-public-proxy-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for immich-public-proxy"
    exit 1
}
printf '%s
...
if incus_exec_stdin "$var_instance" bash -c '
    for i in $(seq 1 30); do
        if docker ps --format "{{.Names}}" 2>/dev/null | grep -qi "immich-public-proxy"; then
            echo "OK"
            exit 0
        fi
        sleep 2
...
' 2>/dev/null | grep -q "^OK$"; then
    echo -e "${GR}Immich Public Proxy deployed on ${var_instance} (${IP})${NC}"
else
    echo -e "${YL}Immich Public Proxy install did not start a Docker container on ${var_instance} (${IP}).${NC}"
