#!/usr/bin/env bash
# ct/alpine-postgresql.sh — Alpine Postgresql
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-postgresql-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-postgresql"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Postgresql deployed on ${var_instance} (${IP})${NC}"
echo ""
