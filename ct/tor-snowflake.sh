#!/usr/bin/env bash
# ct/tor-snowflake.sh — Tor Snowflake
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/tor-snowflake-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for tor-snowflake"
    exit 1
}
...
echo ""
echo -e "${GR}Tor Snowflake deployed on ${var_instance} (${IP})${NC}"
echo ""
