#!/usr/bin/env bash
# ct/owntracks-recorder.sh — Owntracks Recorder
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/owntracks-recorder-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for owntracks-recorder"
    exit 1
}
...
echo ""
echo -e "${GR}Owntracks Recorder deployed on ${var_instance} (${IP})${NC}"
echo ""
