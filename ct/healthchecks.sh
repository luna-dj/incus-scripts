#!/usr/bin/env bash
# ct/healthchecks.sh — Healthchecks
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/healthchecks-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for healthchecks"
    exit 1
}
...
echo ""
echo -e "${GR}Healthchecks deployed on ${var_instance} (${IP})${NC}"
echo ""
