#!/usr/bin/env bash
# ct/passwordpusher.sh — Passwordpusher
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/passwordpusher-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for passwordpusher"
    exit 1
}
...
echo ""
echo -e "${GR}Passwordpusher deployed on ${var_instance} (${IP})${NC}"
echo ""
