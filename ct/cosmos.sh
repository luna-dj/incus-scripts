...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/cosmos-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for cosmos"
    exit 1
}
...
echo ""
echo -e "${GR}Cosmos deployed on ${var_instance} (${IP})${NC}"
echo ""
