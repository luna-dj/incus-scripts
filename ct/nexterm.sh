...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/nexterm-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for nexterm"
    exit 1
}
...
echo ""
echo -e "${GR}Nexterm deployed on ${var_instance} (${IP})${NC}"
echo ""
