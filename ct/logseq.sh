...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/logseq-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for logseq"
    exit 1
}
...
echo ""
echo -e "${GR}Logseq deployed on ${var_instance} (${IP})${NC}"
echo ""
