...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/decypharr-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for decypharr"
    exit 1
}
...
echo ""
echo -e "${GR}Decypharr deployed on ${var_instance} (${IP})${NC}"
echo ""
