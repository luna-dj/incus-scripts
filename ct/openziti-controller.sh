#!/usr/bin/env bash
# ct/openziti-controller.sh — Openziti Controller
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/openziti-controller-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for openziti-controller"
    exit 1
}
...
echo ""
echo -e "${GR}Openziti Controller deployed on ${var_instance} (${IP})${NC}"
echo ""
