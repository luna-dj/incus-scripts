...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/squid-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for squid"
    exit 1
}
...
echo ""
echo -e "${GR}Squid deployed on ${var_instance} (${IP})${NC}"
echo ""
