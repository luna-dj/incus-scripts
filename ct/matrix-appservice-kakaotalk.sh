#!/usr/bin/env bash
# ct/matrix-appservice-kakaotalk.sh — matrix-appservice-kakaotalk
# Matrix bridge to KakaoTalk.
...
#   HS_URL=https://matrix.femdev.nl HS_DOMAIN=femdev.nl bash <(curl ... ct/matrix-appservice-kakaotalk.sh)
...
APP="matrix-appservice-kakaotalk"
var_tags="${var_tags:-matrix;bridge;kakaotalk}"
var_cpu="${var_cpu:-1}"
var_ram="${var_ram:-512}"
var_disk="${var_disk:-4}"
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/matrix-appservice-kakaotalk-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for matrix-appservice-kakaotalk"
    exit 1
}
...
echo ""
echo -e "${GR}matrix-appservice-kakaotalk deployed on ${var_instance} (${IP})${NC}"
echo ""
