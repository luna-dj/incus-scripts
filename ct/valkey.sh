...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/valkey-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for valkey"
    exit 1
}
...
echo ""
echo -e "${GR}Valkey deployed on ${var_instance} (${IP})${NC}"
echo ""
