#!/usr/bin/env bash
# ct/splunk-enterprise.sh — Splunk Enterprise
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/splunk-enterprise-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for splunk-enterprise"
    exit 1
}
...
echo ""
echo -e "${GR}Splunk Enterprise deployed on ${var_instance} (${IP})${NC}"
echo ""
