#!/usr/bin/env bash
# ct/whisparr-eros.sh — Whisparr Eros
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/whisparr-eros-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for whisparr-eros"
    exit 1
}
...
echo ""
echo -e "${GR}Whisparr Eros deployed on ${var_instance} (${IP})${NC}"
echo ""
