#!/usr/bin/env bash
# ct/stirling-pdf.sh — Stirling Pdf
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/stirling-pdf-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for stirling-pdf"
    exit 1
}
...
echo ""
echo -e "${GR}Stirling Pdf deployed on ${var_instance} (${IP})${NC}"
echo ""
