#!/usr/bin/env bash
# ct/alpine-rustdeskserver.sh — Alpine Rustdeskserver
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-rustdeskserver-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-rustdeskserver"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Rustdeskserver deployed on ${var_instance} (${IP})${NC}"
echo ""
