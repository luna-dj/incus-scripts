#!/usr/bin/env bash
# ct/ddns-updater.sh — Ddns Updater
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "https://codeberg.org/luna-dj/incus-scripts/raw/branch/main/install/ddns-updater-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for ddns-updater"
    exit 1
}
...
echo ""
echo -e "${GR}Ddns Updater deployed on ${var_instance} (${IP})${NC}"
echo ""
