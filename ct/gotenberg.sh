...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/gotenberg-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for gotenberg"
    exit 1
}
...
echo ""
echo -e "${GR}Gotenberg deployed on ${var_instance} (${IP})${NC}"
echo ""
