#!/usr/bin/env bash
# ct/alpine-tinyauth.sh — Alpine Tinyauth
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/alpine-tinyauth-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for alpine-tinyauth"
    exit 1
}
...
echo ""
echo -e "${GR}Alpine Tinyauth deployed on ${var_instance} (${IP})${NC}"
echo ""
