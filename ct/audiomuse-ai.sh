#!/usr/bin/env bash
# ct/audiomuse-ai.sh — Audiomuse Ai
...
INSTALL_SCRIPT=$(curl -fsSL --http1.1 "${INCUS_BASE}/install/audiomuse-ai-install.sh" 2>/dev/null) || {
    log_error "Failed to fetch install script for audiomuse-ai"
    exit 1
}
...
echo ""
echo -e "${GR}Audiomuse Ai deployed on ${var_instance} (${IP})${NC}"
echo ""
