#!/usr/bin/env bash
# install/qbittorrent-exporter-install.sh — Qbittorrent Exporter (addon)
...
header_info "Qbittorrent Exporter"
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for Qbittorrent Exporter addon)"
  setup_docker
  msg_ok "Docker installed"
fi

msg_info "Loading upstream addon script for Qbittorrent Exporter"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/qbittorrent-exporter.sh"
...
echo ""
echo -e "${GR}Qbittorrent Exporter installation complete!${NC}"
echo ""
