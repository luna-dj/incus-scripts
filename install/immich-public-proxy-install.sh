#!/usr/bin/env bash
# install/immich-public-proxy-install.sh — Immich Public Proxy (addon)
...
header_info "Immich Public Proxy"
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for Immich Public Proxy addon)"
  setup_docker
  msg_ok "Docker installed"
fi

msg_info "Loading upstream addon script for Immich Public Proxy"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/immich-public-proxy.sh"
...
echo ""
echo -e "${GR}Immich Public Proxy installation complete!${NC}"
echo ""
