#!/usr/bin/env bash
# install/crowdsec-install.sh — CrowdSec (addon)
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for CrowdSec addon)"
  setup_docker
  msg_ok "Docker installed"
fi

msg_info "Loading upstream addon script for CrowdSec"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/crowdsec.sh"
...
echo ""
echo -e "${GR}CrowdSec installation complete!${NC}"
echo ""
