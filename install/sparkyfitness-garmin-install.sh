#!/usr/bin/env bash
# install/sparkyfitness-garmin-install.sh — Sparkyfitness Garmin (addon)
...
header_info "Sparkyfitness Garmin"
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for Sparkyfitness Garmin addon)"
  setup_docker
  msg_ok "Docker installed"
fi

msg_info "Loading upstream addon script for Sparkyfitness Garmin"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/sparkyfitness-garmin.sh"
...
echo ""
echo -e "${GR}Sparkyfitness Garmin installation complete!${NC}"
echo ""
