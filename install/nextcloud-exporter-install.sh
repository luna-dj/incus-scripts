#!/usr/bin/env bash
# install/nextcloud-exporter-install.sh — Nextcloud Exporter (addon)
...
header_info "Nextcloud Exporter"
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for Nextcloud Exporter addon)"
  setup_docker
...
msg_info "Loading upstream addon script for Nextcloud Exporter"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/nextcloud-exporter.sh"
...
echo ""
echo -e "${GR}Nextcloud Exporter installation complete!${NC}"
echo ""
