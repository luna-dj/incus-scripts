#!/usr/bin/env bash
# install/actual-budget-prometheus-exporter-install.sh — Actual Budget Prometheus Exporter (addon)
...
header_info "Actual Budget Prometheus Exporter"
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for Actual Budget Prometheus Exporter addon)"
  setup_docker
...
msg_info "Loading upstream addon script for Actual Budget Prometheus Exporter"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/actual-budget-prometheus-exporter.sh"
...
echo ""
echo -e "${GR}Actual Budget Prometheus Exporter installation complete!${NC}"
echo ""
