#!/usr/bin/env bash
# install/prometheus-paperless-ngx-exporter-install.sh — Prometheus Paperless Ngx Exporter (addon)
...
header_info "Prometheus Paperless Ngx Exporter"
...
if ! command -v docker &>/dev/null; then
  msg_info "Installing Docker (required for Prometheus Paperless Ngx Exporter addon)"
  setup_docker
...
msg_info "Loading upstream addon script for Prometheus Paperless Ngx Exporter"
UPSTREAM_URL="https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/tools/addon/prometheus-paperless-ngx-exporter.sh"
...
echo ""
echo -e "${GR}Prometheus Paperless Ngx Exporter installation complete!${NC}"
echo ""
